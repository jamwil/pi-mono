...
```bash
npm install @jamwil/pi-ai
```

TypeBox exports are re-exported from `@jamwil/pi-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@jamwil/pi-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@jamwil/pi-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@jamwil/pi-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@jamwil/pi-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@jamwil/pi-ai';
...
```typescript
import { getModel, stream } from '@jamwil/pi-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@jamwil/pi-ai';
...
```typescript
import { Model, stream } from '@jamwil/pi-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@jamwil/pi-ai';
...
```typescript
import { getModel, complete, Context } from '@jamwil/pi-ai';
...
```typescript
import { Context, getModel, complete } from '@jamwil/pi-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@jamwil/pi-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@jamwil/pi-ai';
...
```bash
npx @jamwil/pi-ai login              # interactive provider selection
npx @jamwil/pi-ai login anthropic    # login to specific provider
npx @jamwil/pi-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@jamwil/pi-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@jamwil/pi-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@jamwil/pi-ai/oauth';
...
```typescript
import { getModel, complete } from '@jamwil/pi-ai';
import { getOAuthApiKey } from '@jamwil/pi-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@jamwil/pi-ai`
