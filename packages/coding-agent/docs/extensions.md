...
| Package | Purpose |
|---------|---------|
| `@jamwil/pi` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `@sinclair/typebox` | Schema definitions for tool parameters |
| `@jamwil/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@jamwil/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@jamwil/pi";
...
```typescript
import { isBashToolResult } from "@jamwil/pi";
...
```typescript
import { createLocalBashOperations } from "@jamwil/pi";
...
```typescript
import { SessionManager } from "@jamwil/pi";
...
```typescript
import type { AutocompleteItem } from "@jamwil/pi-tui";
...
```typescript
import { withFileMutationQueue } from "@jamwil/pi";
...
import { Type } from "@sinclair/typebox";
import { StringEnum } from "@jamwil/pi-ai";
import { Text } from "@jamwil/pi-tui";
...
**Important:** Use `StringEnum` from `@jamwil/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@jamwil/pi";
...
```typescript
import { createBashTool } from "@jamwil/pi";
...
```typescript
import { Text, Component } from "@jamwil/pi-tui";
...
import { matchesKey } from "@jamwil/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@jamwil/pi";
