...
Pi fetches `https://pi.dev/api/latest-version` at startup to look for a newer version.

Set `PI_SKIP_VERSION_CHECK=1` to disable that request. Use `--offline` or `PI_OFFLINE=1` to disable all startup network operations described here, including Pi update checks and package update checks.
