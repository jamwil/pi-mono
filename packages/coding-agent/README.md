...
```bash
npm install -g --ignore-scripts @jamwil/pi-coding-agent
```
...
Pi checks `https://pi.dev/api/latest-version` at startup to see whether a newer version exists. Disable that request with `PI_SKIP_VERSION_CHECK=1`.

Use `--offline` or `PI_OFFLINE=1` to disable all startup network operations described here, including Pi update checks and package update checks.
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@jamwil/pi-coding-agent";
...
| `PI_OFFLINE` | Disable startup network operations, including Pi update checks and package update checks |
