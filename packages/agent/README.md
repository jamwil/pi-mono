# @jamwil/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@jamwil/pi-ai`.

## Installation

```bash
npm install @jamwil/pi-agent-core
```

## Quick Start

```typescript
import { Agent } from "@jamwil/pi-agent-core";
...
```typescript
declare module "@jamwil/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@jamwil/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@jamwil/pi-agent-core";
